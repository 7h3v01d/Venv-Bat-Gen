#!/usr/bin/env python3
"""
Venv Batch Template Generator  (PyQt6 edition)
KeystoneAI Pty Ltd
...
class App(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        sub = QLabel(
            "Generate run.bat · pip.bat · shell.bat · sync.bat · doctor.bat for any Python project."
        )
    ...
    def _build_entry_group(self) -> QGroupBox:
        ...
        entry_row.addWidget(QLabel("Entry value:"))
        self._app_entry = QLineEdit("main.py")
        entry_row.addWidget(self._app_entry, 1)
        btn_browse_py = QPushButton("Browse .py…")
        btn_browse_py.clicked.connect(self._browse_entry_file)
        entry_row.addWidget(btn_browse_py)
        layout.addLayout(entry_row)

        hint = QLabel(
            'File mode: "main.py" or "src\\\\app.py"   ·   Module mode: "desktop_companion"'
        )
    ...
    def _read_config(self) -> GeneratorConfig:
        ...
        entry_mode   = "module" if self._entry_mode_module.isChecked() else "file"
        ...
        if entry_mode == "module":
            if not MODULE_RE.match(app_entry):
                raise ValueError(
                    "Module mode expects a valid Python module name, "
                    "e.g. desktop_companion or package.submodule.\n\n"
    ...
    def _on_entry_mode_changed(self, checked: bool) -> None:
        ...
        if self._entry_mode_module.isChecked():
            # Switching TO module mode — replace known file defaults
            if current in FILE_DEFAULTS or current.endswith(".py"):
                self._app_entry.setText("desktop_companion")
            self._app_entry.setPlaceholderText("e.g. desktop_companion")
        else:
            # Switching TO file mode — replace known module defaults
            if current in MODULE_DEFAULTS:
    ...
    def _refresh_preview(self) -> None:
        try:
            ...
            self._status(f"Preview updated — {cfg.project_name}  [{cfg.entry_mode}: {cfg.app_entry}]")
    ...
    def _on_generate(self) -> None:
        ...
        self._log_line(f"Mode: {cfg.entry_mode}  ·  Entry: {cfg.app_entry}", "")
